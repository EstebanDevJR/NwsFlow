...
function readRequiredSecret(name) {
    ...
    if (!v || v.length < MIN_JWT_LEN) {
        throw new Error(`${name} must be set and at least ${MIN_JWT_LEN} characters. Generate a strong random value (e.g. openssl rand -base64 48).`);
    }
...
export function resetJwtCachesForTests() {
    _jwtSecret = null;
